...
出差期间他的**当地时区跟着目的地走**：每日作息表、状态栏、每日卡片，以及他说
「今天/今晚/该睡了」时的判断，全部按目的地时间算；**你始终按北京时间**，所以你俩
的时差会变，他也知道这一点。他不会刻意宣布行程，只在自然相关时（时差、窗外、手边
的事）带一句。

作息表也会换成出差版本——在酒店里睡觉、清晨倒时差看邮件、白天在会议或工坊、
夜里在那座城市。出发和返程会让当天剩下的日程作废重建，所以他不会一边「在伦敦书房」
一边说自己人在东京。晚上照样是完整的可聊时段：人在外地不等于找不到他。
...
他会隔一段时间自己决定要不要给你发条私信（默认每 8 小时检查一次，15% 概率，
且你至少安静 90 分钟之后才考虑）。他睡着的时段本来就不会发。

用 `/proactive_dm` 调，全部即时生效并写进 `bot_config`：

| 参数 | 作用 |
|---|---|
| `启用` | 总开关。关掉后他完全不会主动私信，你找他照常回 |
| `检查间隔小时` / `发送概率` | 多久考虑一次、每次真正发出去的概率 |
| `最少安静分钟` | 你安静满多久之后他才考虑开口 |
| `深夜允许` / `深夜概率` | 北京时间 2:00–8:00 那一档单独控制 |
| `立即发送` | 马上让他发一条，不受概率和安静时长限制（用来试效果） |

## 状态栏

他的 Discord 状态栏跟着当天的作息表走：时段一换就换一个固定活动（看晨报、
家族办公室的文件、在书房……），时段开头还可能挂一句 AI 现编的碎碎念，挂满设定的
时长就落回那个固定活动。**只有碎碎念要花 API 额度**，固定活动是白嫖的。

用 `/presence` 调：

| 参数 | 作用 |
|---|---|
| `启用轮换` | 关掉后状态栏定格在当前这个，不再自动更换 |
| `检查间隔分钟` | 后台多久检查一次状态栏 |
| `瞬时状态` | 碎碎念气泡的总开关。**关掉后完全不为状态栏调用 AI**，最省额度的常驻选项 |
| `瞬时概率` / `瞬时时长分钟` | 碎碎念多久出现一次、一次挂多久 |
| `立刻换一个` | 立刻重新挑一个挂上 |
| `清空状态栏` | 清空并停止轮换，什么都不显示也不调用 AI；想恢复用 `启用轮换:True` |

停止轮换和清空都只影响 Discord 那一栏——他提示词里「此刻在做什么」照常更新，
所以聊天内容不会因此变得不对劲。
...
- **关掉主动私信**：在 Discord 里跑 `/proactive_dm 启用:False`。
- **省 API 额度**：`/presence 瞬时状态:False` 就不再为状态栏调用 AI；要更彻底就 `/presence 清空状态栏:True`。
- **关掉每日主动话题**：在 Discord 里跑 `/主动话题设置 启用:False`，或者干脆不往频道池里加频道。
- **关掉承诺跟进**：在 `events.py` 的 `on_ready` 里注释掉 `tasks_bg.commitment_followup_check.start()`。
...
除了在服务器和私聊里正常对话之外，配好数据库后他还会：

- **记事**：从你的话里自动提取值得记的细节（喜好、健康、计划、日期），并随时间自然淡忘不重要的部分。
- **有作息**：按他所在地的时间生活（出差时跟着目的地走）——会睡觉（深夜基本不回消息）、会上班、醒来后会补看你留的话。忙的时候可能先给你一个表情，稍后再认真回。状态栏也跟着作息走，用 `/presence` 调。
- **会出差**：偶尔离开伦敦几天，时区和作息跟着目的地变。用 `/trip` 查看或调整。
- **每日主动话题**：在你指定的频道池里，每天挑随机时间主动找你聊一两句。用 `/主动话题设置` 配频道、次数、时段、最短间隔和话题偏好。
- **承诺跟进**：他答应过你的事会记进「账本」，到点回来找你。也可以右键消息手动记一笔。
- **服务器管理**：建帖、开投票、发贴纸、转发消息、创建活动、加身份组、发金币经验等（见 `prompts.py` 的管理能力章节）。
- **甩链接**：聊到歌、书、影视、游戏、词条时会附上对应平台的搜索链接（Spotify / Apple Music / YouTube / B站 / 豆瓣 / Steam / IMDb / 维基 / GitHub 等）。
- **主动私信**：隔一段时间自己决定要不要找你说句话。用 `/proactive_dm` 调频率、概率和深夜档，也可以整个关掉。
- **每日状态卡片、节日感言**：可以在 `/post_config` 里调，也可以整个关掉。
...
slash_cmds.py       所有 /命令
tasks_bg.py         后台 tasks.loop（主动私信、每日主动话题、纪念日、每日卡片、清理）
memory.py           长期记忆、提醒、每日摘要
life_state.py       每日作息表与「他此刻在做什么」的状态推进
followups.py        承诺跟进账本（他答应过的事到点会回来找你）
polls.py            投票结束后的自动跟进
context_menus.py    消息右键菜单（记进账本 / 转发到主频道等）
...
- 承诺跟进账本（`commitment_followups` 表，他答应过的事到点会回来找你）
- 每日作息表（`daily_life_schedules` 表，决定他此刻「在做什么」）
- 投票跟进（`poll_followups` 表，投票结束后他会回来说说结果）
